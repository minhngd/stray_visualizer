...
def read_data(flags):
    ...
    rgb_frames = [f for f in rgb_frames if '.npy' in f or '.png' in f]
...
def load_depth(path, confidence=None, filter_level=0):
    ...
    # depth_m = depth_m.reshape(256, 192)
...
def get_intrinsics(intrinsics):
    ...
    print("sjfskjfsd: \n",intrinsics)
    return o3d.camera.PinholeCameraIntrinsic(width=480, height=640, fx=intrinsics[0, 0],
        fy=intrinsics[1, 1], cx=intrinsics[0, 2], cy=intrinsics[1, 2])
# def get_intrinsics(intrinsics):
#     """
#     Scales the intrinsics matrix to be of the appropriate scale for the depth maps.
#     """
...
def point_clouds(flags, data):
    ...
    """
    pcs = []
    intrinsics_ = get_intrinsics(data['intrinsics'])
    print
    ...
    for i, (T_WC) in enumerate(zip(data['poses'])):
        if i < 200:
            ...
            pc += o3d.geometry.PointCloud.create_from_rgbd_image(rgbd, intrinsics_, extrinsic = T_CW)
    return [pc]
